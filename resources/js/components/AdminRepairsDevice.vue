        ...
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            <tr v-for="rep in repairs" :key="rep.id" class="text-center">
              <td>
                <a
                  :href="`/naprawa/`+rep.devices.brand.category.slug+`/`+rep.devices.brand.slugi+`/`+rep.devices.slugii+'/'+rep.id+'/'+rep.slugi_repair"
                >{{rep.name}}</a>
              </td>
              <td>{{rep.users.email}}</td>
              <td>
...
export default {
  ...
  methods: {
    ...
    odrzuc(id) {
      Swal.fire({
        title: "Na pewno chcesz odrzucić naprawę?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    przekazdorealziacjii(id) {
      Swal.fire({
        ...
        title: "Oops...",
        text:
          'Nie można zmienić statusu na "W realizacji", kiedy naprawa jest zaakceptowana.',
      });
    },
    akceptujj(id) {
      Swal.fire({
        ...
        title: "Oops...",
        text:
          "Nie można zaakceptować naprawy , kiedy naprawa jest w realizacji.",
      });
    },
    deleterepair(id) {
      Swal.fire({
